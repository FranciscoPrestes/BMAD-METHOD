...
/**
 * Beat Method CLI - Direct execution wrapper for npx
...
if (isNpxExecution) {
  ...
  const beatCliPath = path.join(__dirname, 'cli', 'beat-cli.js');

  if (!fs.existsSync(beatCliPath)) {
    console.error('Error: Could not find beat-cli.js at', beatCliPath);
  ...
  try {
    ...
    execSync(`node "${beatCliPath}" ${args.join(' ')}`, {
      stdio: 'inherit',
