...
**Module:** Beat Builder (beat-builder)

## Purpose

ReDoc is an intelligent documentation maintenance system for BEAT modules, workflows, and agents. It uses a reverse-tree approach (leaf folders first, then parent folders) to systematically generate and update README.md files with technical writer quality output.

The workflow understands BEAT conventions deeply and focuses documentation on distinctive features rather than explaining standard patterns, resulting in succinct, precise technical documentation.
...
- **Convention-Aware**: Loads BEAT architecture patterns and only documents unique/distinctive aspects
...
- **Module path**: `{beat_folder}/bmm` (documents entire module: root, workflows, agents)
- **Workflows folder**: `{beat_folder}/beat-method/workflows` (documents all workflows)
- **Agents folder**: `{beat_folder}/beat-method/agents` (documents all agents)
- **Single workflow**: `{beat_folder}/beat-method/workflows/product-brief` (documents one workflow)
- **Single agent**: `{beat_folder}/beat-method/agents/prd-agent.md` (documents one agent)
...
1. **Initialize**: Load BEAT conventions and validate target
...
- **Context-Aware**: Uses BEAT convention knowledge to highlight only distinctive features
