# BEAT Agent Architecture Reference
...
- `id` - Unique path identifier (e.g., "{beat_folder}/beat-method/agents/analyst.md")
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/{beat_folder}/{module}/config.yaml and set variables</i>
...
```xml
<module-integration>
  <module-path>{project-root}/{beat_folder}/{module-code}</module-path>
  <config-source>{module-path}/config.yaml</config-source>
  <workflows-path>{project-root}/{beat_folder}/{module-code}/workflows</workflows-path>
</module-integration>
```
...
```
Good: {project-root}/{beat_folder}/{module}/agents/
...
```xml
<!-- Full path -->
<item cmd="*create-prd" run-workflow="{project-root}/{beat_folder}/beat-method/workflows/prd/workflow.yaml">
...
```xml
<item cmd="*validate" exec="{project-root}/{beat_folder}/core/tasks/validate-workflow.xml">
  Validate document
</item>
```
...
```xml
<item cmd="*standup"
   exec="{project-root}/{beat_folder}/beat-method/tasks/daily-standup.xml"
...
<item cmd="*run" exec="{project-root}/{beat_folder}/module/task.md">
...
**Workflow Interaction Styles** (BEAT v6 default):
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/{beat_folder}/{module}/config.yaml</i>
  <i>Remember the users name is {user_name}</i>
  <i>ALWAYS communicate in {communication_language}</i>
</critical-actions>
```
...
```xml
<agent id="{beat_folder}/{module}/agents/{name}.md"
       name="{Name}"
       title="{Title}"
       icon="{emoji}">
  <persona>...</persona>
