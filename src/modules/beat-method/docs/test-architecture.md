...
TEA is the only BMM agent with its own top-level module directory (`beat-method/testarch/`):
...
**To disable**: Set `tea_use_mcp_enhancements: false` in `{beat_folder}/beat-method/config.yaml` OR remove MCPs from IDE config.
