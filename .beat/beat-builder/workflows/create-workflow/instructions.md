...
<critical>The workflow execution engine is governed by: {project-root}/.beat/core/tasks/workflow.xml</critical>
<critical>You MUST have already loaded and processed: {project-root}/.beat/beat-builder/workflows/create-workflow/workflow.yaml</critical>
...
- Workflow: {project-root}/.beat/core/workflows/brainstorming/workflow.yaml
...
- What module will it belong to? (e.g., "beat-method", "beat-builder", "cis")
...
- Author name (default to user_name or "Beat")
...
config_source: '{project-root}/.beat/{{target_module}}/config.yaml'
...
# Critical variables from config
config_source: '{project-root}/.beat/module/config.yaml'
output_folder: '{config_source}:output_folder'
user_name: '{config_source}:user_name'
communication_language: '{config_source}:communication_language'
date: system-generated
...
1. ALWAYS include critical headers:
   - Workflow engine reference: {project-root}/.beat/core/tasks/workflow.xml
...
4. Use proper XML tags from guide:
   ...
   - Output: <template-output>, <invoke-task halt="true">{project-root}/.beat/core/tasks/adv-elicit.xml</invoke-task>, <critical>, <example>
...
3. List ALL referenced files by scanning:
   ...
   **Critical: Workflow Dependencies**
   ...
   - Example: `<invoke-workflow>{project-root}/.beat/core/workflows/x/workflow.yaml</invoke-workflow>`
     → Add ".beat/core/workflows/x/workflow.yaml" to web_bundle_files
...
web_bundle:
  ...
  existing_workflows:
    - workflow_variable_name: '.beat/path/to/workflow.yaml'
```
...
web_bundle:
  ...
  author: 'Beat'
  instructions: '.beat/beat-method/workflows/brainstorm-game/instructions.md'
  template: false
  web_bundle_files:
    - '.beat/beat-method/workflows/brainstorm-game/instructions.md'
    - '.beat/mmm/workflows/brainstorm-game/game-context.md'
    - '.beat/core/workflows/brainstorming/workflow.yaml'
  existing_workflows:
    - core_brainstorming: '.beat/core/workflows/brainstorming/workflow.yaml'
```
...
- Next steps:
  - Run the BEAT Method installer to this project location
