...
<critical>You MUST have already loaded and processed: {project-root}/.beat/beat-builder/workflows/create-module/workflow.yaml</critical>
<critical>Study existing modules in: {project-root}/.beat/ for patterns</critical>
...
<check if="create">
  <action>Invoke module-brief workflow: {project-root}/.beat/beat-builder/workflows/module-brief/workflow.yaml</action>
...
2. **Module code** - Generate kebab-case from name following patterns:
   ...
   - Present suggested code and confirm it works for paths like .beat/{{code}}/agents/
...
# EXAMPLE Static path:
# data_path:
#   result: "{project-root}/.beat/{{module_code}}/data"
...
The module can be configured in `.beat/{{module_code}}/config.yaml`
