 ...
 * Called by BEAT installer when processing the module
 */
...
async function setupConfigurations(config) {
  ...
  const configPath = path.join(config.project_root, 'beat', config.module_code, 'config.json');
...
// Export functions for BEAT installer
module.exports = {
  installModule,
  initDatabase,
  generateSamples,
