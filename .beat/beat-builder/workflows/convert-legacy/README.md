...
- Ensures all paths use `{project-root}/.beat/` format
...
- Maps `elicit: true` flags to `<invoke-task halt="true">{project-root}/.beat/core/tasks/adv-elicit.xml</invoke-task>` tags
...
- **Solution**: Ensure all references use `{project-root}/.beat/` format
...
- Review the workflow creation guide at `/.beat/beat-builder/workflows/create-workflow/workflow-creation-guide.md`
- Check conversion mappings at `/.beat/beat-builder/data/v4-to-v6-mappings.yaml`
