...
- `id` - Unique path identifier (e.g., ".beat/beat-method/agents/analyst.md")
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/.beat/{module}/config.yaml and set variables</i>
...
```xml
<module-integration>
  <module-path>{project-root}/.beat/{module-code}</module-path>
  <config-source>{module-path}/config.yaml</config-source>
  <workflows-path>{project-root}/.beat/{module-code}/workflows</workflows-path>
</module-integration>
```
...
```
Good: {project-root}/.beat/{module}/agents/
...
```xml
<!-- Full path -->
<item cmd="*create-prd" run-workflow="{project-root}/.beat/beat-method/workflows/prd/workflow.yaml">
...
```xml
<item cmd="*validate" exec="{project-root}/.beat/core/tasks/validate-workflow.xml">
  Validate document
</item>
```
...
```xml
<item cmd="*standup"
   exec="{project-root}/.beat/beat-method/tasks/daily-standup.xml"
...
<item cmd="*run" exec="{project-root}/.beat/module/task.md">
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/.beat/{module}/config.yaml</i>
  <i>Remember the users name is {user_name}</i>
  <i>ALWAYS communicate in {communication_language}</i>
</critical-actions>
```
...
```xml
<agent id=".beat/{module}/agents/{name}.md"
       name="{Name}"
       title="{Title}"
       icon="{emoji}">
  <persona>...</persona>
