...
Live in their own dedicated directories under `.beat/agents/`:
...
Part of a module system under `.beat/{module}/agents/`:

```
.beat/beat-method/agents/
...
**Location:** `.beat/agents/{agent-name}/`
...
**Location:** `.beat/agents/{agent-name}/` with sidecar directory
...
**Location:** `.beat/{module}/agents/`

**Characteristics:**

- Part of a BEAT module (beat-method, beat-builder, cis)
...
agent:
  ...
  persona:
    ...
    principles: ['...']
  critical_actions:
    - 'Load config from {project-root}/.beat/{module}/config.yaml'
  menu:
    - trigger: create-prd
...
<agent id=".beat/beat-method/agents/pm.md" name="John" title="Product Manager" icon="📋">
  ...
  <critical-actions>
    <i>Load config from {project-root}/.beat/{module}/config.yaml</i>
  </critical-actions>
  <menu>
    <item cmd="*help">Show numbered menu</item>
    <item cmd="*create-prd" run-workflow="{project-root}/.beat/beat-method/workflows/prd/workflow.yaml">Create PRD</item>
    <item cmd="*validate" exec="{project-root}/.beat/core/tasks/validate-workflow.xml">Validate document</item>
    <item cmd="*exit">Exit</item>
  </menu>
</agent>
```
