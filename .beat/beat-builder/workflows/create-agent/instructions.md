...
<critical>You MUST have already loaded and processed: {project-root}/.beat/beat-builder/workflows/create-agent/workflow.yaml</critical>
<critical>Study YAML agent examples in: {project-root}/.beat/beat-method/agents/ for patterns</critical>
...
<step n="-1" goal="Optional brainstorming for agent ideas" optional="true">
  ...
  <check if="user answered yes">
    <action>Invoke brainstorming workflow: {project-root}/.beat/core/workflows/brainstorming/workflow.yaml</action>
...
**Path Determination:**
  ...
  <check if="module agent selected">
    <action>Discover which module system fits best (beat-method, beat-builder, cis, or custom)</action>
    <action>Store as {{target_module}} for path determination</action>
    <note>Agent will be saved to: .beat/{{target_module}}/agents/</note>
  </check>

  <check if="standalone agent selected">
    <action>Explain this will be their personal agent, not tied to a module</action>
    <note>Agent will be saved to: .beat/agents/{{agent-name}}/</note>
    <note>All sidecar files will be in the same folder</note>
  </check>

<critical>Determine agent location:</critical>

- Module Agent → .beat/{{module}}/agents/{{agent-name}}.agent.yaml
- Standalone Agent → .beat/agents/{{agent-name}}/{{agent-name}}.agent.yaml
...
- trigger: [another capability]
  workflow: "{project-root}/.beat/SOURCE_MODULE/workflows/path/to/workflow.yaml"
  workflow-install: "{project-root}/.beat/THIS_MODULE/workflows/vendored/path/workflow.yaml"
...
<example type="yaml">
  agent:
    metadata:
      id: .beat/{{target_module}}/agents/{{agent_filename}}.md
