...
graph TB
    ...
    subgraph BeatMethod["<b>BEAT METHOD (Recommended)</b>"]
        direction TB
    ...
    subgraph Enterprise["<b>ENTERPRISE METHOD</b>"]
        direction TB
        EntNote["<b>Uses Beat Method Planning</b><br/>+<br/>Extended Phase 3 workflows<br/>(Architecture + Security + DevOps)<br/>30+ stories typically"]
    end
    ...
    Enterprise -.->|Uses Beat planning| Epics
    ...
    style BeatMethod fill:#e1bee7,stroke:#6a1b9a,stroke-width:3px,color:#000
...
| **prd**                      | PM            | Beat Method | Strategic PRD                              | 10-50+          |
| **gdd**                      | Game Designer | Beat Method | Game Design Document                       | 10-50+          |
| **narrative**                | Game Designer | Beat Method | Story-driven game/experience design        | 10-50+          |
| **create-epics-and-stories** | PM            | Beat Method | Break PRD/GDD into Epic+Stories            | N/A             |
| **ux**                       | UX Designer   | Beat Method | Optional UX specification                  | N/A             |
...
### Track 2: Beat Method (RECOMMENDED)
...
**Planning (Phase 2):** Uses Beat Method planning (PRD + Epic+Stories)
...
**Purpose:** Strategic PRD with epic breakdown for software products (Beat Method track).
...
**Purpose:** Complete game design document for game projects (Beat Method track).
...
**Purpose:** Story-driven design workflow for games/experiences where narrative is central (Beat Method track).
...
**Purpose:** UX specification for projects where user experience is the primary differentiator (Beat Method track).
...
**Purpose:** Break PRD/GDD requirements into bite-sized stories organized in epics (Beat Method track).
...
- **Software product** → `prd` (Beat Method)
- **Game (gameplay-first)** → `gdd` (Beat Method)
- **Game (story-first)** → `narrative` + `gdd` (Beat Method)
- **UX innovation project** → `ux` + `prd` (Beat Method)
...
| PRD.md              | **architecture** (Level 3-4)         | Beat Method (recommended)    |
| GDD.md              | **architecture** (game tech)         | Beat Method (recommended)    |
| narrative-design.md | **architecture** (narrative systems) | Beat Method                  |
| ux-spec.md          | **architecture** (frontend design)   | Beat Method                  |
...
- **Beat Method:** Optional Phase 3 (simple), Required Phase 3 (complex)
...
If `workflow-init` suggests Beat Method, there's likely complexity you haven't considered. Review carefully before overriding.
...
### Greenfield Software (Beat Method)
...
### Brownfield Software (Beat Method)
...
A: No. Simple changes use `tech-spec` (Quick Flow). Only Beat Method and Enterprise tracks create PRDs.

**Q: Can I skip Phase 3 (Solutioning)?**
A: Yes for Quick Flow. Optional for Beat Method (simple projects). Required for Beat Method (complex projects) and Enterprise.
