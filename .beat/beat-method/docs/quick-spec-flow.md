...
Quick Spec Flow is a **streamlined alternative** to the full Beat Method for Quick Flow track projects. Instead of going through Product Brief → PRD → Architecture, you go **straight to a context-aware technical specification** and start coding.
...
❌ **Use Beat Method or Enterprise tracks when:**
...
| Aspect                | Quick Flow Track             | Beat Method/Enterprise Tracks      |
| --------------------- | ---------------------------- | ---------------------------------- |
...
## When to Graduate from Quick Flow to Beat Method

Start with Quick Flow, but switch to Beat Method when:
...
💡 **Tip:** You can always run `workflow-init` later to transition from Quick Flow to Beat Method!
...
- Beat Method installed (`npx beat-method install`)
...
**A:** No problem! You can always transition to Beat Method by running workflow-init and create-prd. Your tech-spec becomes input for the PRD.
