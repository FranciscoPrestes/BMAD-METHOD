...
**Required for:** Beat Method (complex projects), Enterprise Method

**Optional for:** Beat Method (simple projects), Quick Flow (skip entirely)
...
graph TB
    ...
    subgraph BeatEnterprise["<b>BEAT METHOD + ENTERPRISE (Same Start)</b>"]
        direction TB
    ...
    FromPlanning -->|Beat Method<br/>or Enterprise| Architecture
    ...
    style BeatEnterprise fill:#90caf9,stroke:#0d47a1,stroke-width:3px,color:#000
...
| -------------------------- | --------- | ------------------------ | ------------------------------------------- |
| **architecture**           | Architect | Beat Method, Enterprise  | Technical architecture and design decisions |
| **solutioning-gate-check** | Architect | Beat Complex, Enterprise | Validate planning/solutioning completeness  |
...
- **Beat Method:** Multi-epic projects need architecture to prevent conflicts
- **Enterprise:** Same as Beat Method, plus optional extended workflows (test architecture, security architecture, devops strategy) added AFTER architecture but BEFORE gate check
...
- Multi-epic projects (Beat Complex, Enterprise)
...
- Beat Method Simple with straightforward tech stack
...
- **Always** before Phase 4 for Beat Complex and Enterprise projects
...
- Beat Simple (no gate check required)
...
```
Planning (prd by PM - same as Beat Method)
  → architecture (Architect)
...
**Note:** Enterprise uses the same planning and architecture as Beat Method. The only difference is optional extended workflows added AFTER architecture but BEFORE gate check.
...
Don't over-architect simple projects. Beat Simple projects need simple architecture.
...
- **Planning:** prd (PM) - same as Beat Method
...
**Key Difference:** Enterprise adds optional extended workflows AFTER architecture but BEFORE gate check. Everything else is identical to Beat Method.
...
- Use architecture for Beat Method and Enterprise (both required)
...
A: No. Quick Flow skips it. Beat Method and Enterprise both require it.

**Q: How do I know if I need architecture?**
A: If you chose Beat Method or Enterprise track in planning (workflow-init), you need architecture to prevent agent conflicts.
...
A: Only for Quick Flow. Beat Method and Enterprise both require gate check before Phase 4.
...
A: Beat Method: 1-2 days for architecture. Enterprise: 2-3 days total (1-2 days architecture + 0.5-1 day optional extended workflows). If taking longer, you may be over-documenting.
